...
int createTcpClient(
    const std::string& host, int portNo, const std::string& cert)
{
...
int createUdpClient(
    const std::string& host, int portNo, const std::string& cert)
{
    ztls::CUdpClient udpClient(host, portNo);

    if (!udpClient.Connect())
        return showError(udpClient.GetLastError());
...
//        return showError(sslClient.GetLastError());
    ...
    if (!sslClient.CreateSSL(udpClient.UdpSocket(), udpClient.ServerAddr()))
        return showError(sslClient.GetLastError());
...
int main(int argc, char *argv[])
{
    ...
    createUdpClient(host, portNo, cert);
//    createTcpClient(host, portNo, cert);
